...
"${COMPOSE[@]}" exec -T collector-db mariadb -uroot -p"${MARIADB_ROOT_PASSWORD}" <<SQL
...
GRANT ALL PRIVILEGES ON \`${COLLECTOR_DATABASE_NAME:-ao3_collector}\`.* TO '${COLLECTOR_DATABASE_USER:-collector}'@'%';
FLUSH PRIVILEGES;
SQL
gunzip -c "$BACKUP/collector.sql.gz" | "${COMPOSE[@]}" exec -T collector-db \
...
printf 'Starting application services and running migrations...\n'
"${COMPOSE[@]}" up -d --wait api collector-worker planner-worker export-worker web
