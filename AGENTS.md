...
- Remote: `origin` = `https://github.com/billsargent/ao3-scraper2.git`, branch `main`. Local `main` is in sync with `origin/main` at `cb82f17` ("Add Tag archive: prioritized collection by AO3 tag").
- Verified: `npm run check` (89 tests — unit + MariaDB integration when `COLLECTOR_DATABASE_URL` is set), `web:build`, `npm run test:e2e` (6 Playwright tests), image build, container start + health, high-ID job through the workers, backup/restore drill, docs links/env-sync, shell syntax, and `scripts/tag-smoke.ts` (end-to-end tag queue smoke against the dev DB).
- API additions since the single-container rework: `GET /api/statistics` (archive aggregates for the Overview), `POST /api/exports/:id/verify` (re-hash the on-disk package), and per-source `todayUsage { requests, bytes }` on `GET /api/sources` (shown as Requests/Bandwidth today on the dashboard, plus a dashboard pause/resume kill-switch). Tag archive endpoints: `GET /api/tags/search`, `GET /api/tags/local`, `GET|POST /api/tags/subscriptions`, `PUT|DELETE /api/tags/subscriptions/:id`, `POST /api/tags/subscriptions/:id/queue`.
...
- **Tag archive** (prioritized collection by AO3 tag; migration `0015`): a `tag_subscriptions` table per source (unique `(source_id, tag_slug)`, percent-encoded AO3 slug, `nextPage` cursor, `enabled` flag). The collector worker crawls each enabled subscription's `/tags/<slug>/works` listing one page per 10-minute cycle (newest first), keeps only works the archive doesn't already have (`CollectorStore.filterUncollected` — not collected, not queued, not known gone), and queues them as explicit-IDs jobs, so tag discovery never re-queues known work. Live tag search (`/tags/search`) is one polite fetch; local search runs over the `tags` table. UI page "Tag archive" (presets M/M, F/F, F/M, Gen, Multi + search + queue-now + per-subscription auto-fill toggle). Smoke test: `scripts/tag-smoke.ts` (fake local AO3, no real requests).
